...
namespace octet {
  ...
  public:
    ...
    void make_color(const vec4 &color, bool bumpy, bool shiny) {
      ...
      shininess = new param(vec4(127.5f/255, 0, 0, 0));
    }

    void render(bump_shader &shader, const mat4t &cameraToProjection, const mat4t &modelToCamera, vec4 *light_uniforms, int num_light_uniforms, int num_lights, int draw_shadow_map = 0, int draw_shadow = 0, const mat4t &modelToLight = mat4t()) const {
      shader.render(cameraToProjection, modelToCamera, light_uniforms, num_light_uniforms, num_lights, draw_shadow_map, draw_shadow, modelToLight);
      bind_textures();
    }
